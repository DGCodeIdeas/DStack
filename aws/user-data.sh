...
# --- RDS mode: disable local mysql, point phpMyAdmin at RDS ---
if [ -n "${RDS_ENDPOINT}" ]; then
    log "RDS mode — generating compose override to disable local mysql..."
    cat > "${COMPOSE_OVERRIDE}" <<EOF
# Auto-generated by user-data.sh — RDS mode.
# Disables the bundled mysql container and points phpMyAdmin at RDS.
services:
  mysql:
    profiles: ["disabled"]
  phpmyadmin:
...
EOF
    ...
    ok "RDS override generated."
else
    # Local-mysql mode — ensure no stale override lingers on re-runs
...
log "  Docker Compose stack: $(sudo -u www-data env DOCKER_CONFIG="${DOCKER_CONFIG_DIR}" docker compose --env-file "${DOCKER_ENV_FILE}" -f "${COMPOSE_FILE}" ps --status running -q 2>/dev/null | wc -l) running"
if [ "${CHADA_DIGITAL_ENABLED}" = "true" ]; then
