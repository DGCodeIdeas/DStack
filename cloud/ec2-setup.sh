...
if [[ -n "${DOMAIN}" && -n "${EMAIL_FOR_LETSENCRYPT}" ]]; then
    log "Creating docker-compose.override.yml for SSL..."
    cat > "${REPO_DIR}/docker/docker-compose.override.yml" <<'EOF'
# SSL Override for production with Let's Encrypt
# This file is auto-generated by ec2-setup.sh when DOMAIN and EMAIL_FOR_LETSENCRYPT are set

services:
  nginx:
    volumes:
      - ./ssl:/etc/nginx/ssl:ro
      - ./nginx-vhosts.conf:/etc/nginx/conf.d/default.conf:ro
    ports:
      - "80:80"
      - "443:443"
    environment:
